...
Simply download the `pgboom` script and make it executable.

`pgboom` requires Python 3.6 or later. There is no external requirements except
[psycopg2](https://www.psycopg.org).

On non-Linux platforms, you may have issues with psycopg2 installation, which
depends on libpq. In this case, use `psycopg2-binary` package:
```
...
* The `diff` action relies on `diff` utility which may not be present on Windows.
